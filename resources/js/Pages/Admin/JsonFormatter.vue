...
<template>
    <AdminLayout title="JSON Formatter Analytics">
        <div class="space-y-8 animate-fade-in">
            <!-- Header -->
            <div>
                <h3 class="text-2xl font-bold text-white mb-1">JSON Formatter Traffic</h3>
                <p class="text-gray-400 text-sm font-medium">Monitoring client-side JSON formatting traffic and user activity logs.</p>
            </div>

            <!-- Stats -->
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div v-for="s in [
                    { label: 'Total Page Hits', value: stats.totalHits, color: 'blue', icon: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z' },
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-850/60 text-sm">
                            <tr v-for="log in recentVisits" :key="log.id" class="hover:bg-gray-800/20 transition-colors group">
                                <td class="py-4 font-mono text-xs font-bold text-blue-400">
                                    {{ log.ip_address }}
                                </td>
                                <td class="py-4">
                                    <div v-if="log.user">
