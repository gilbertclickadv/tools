        ...
        new Chart(trafficChartRef.value.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: 'rgba(99, 102, 241, 0.1)', // Indigo-500 tint
...
<template>
    <AdminLayout title="CSV ⇆ JSON Analytics">
        <div class="space-y-8 animate-fade-in font-jakarta">
            <!-- Header -->
            <div>
                <h3 class="text-2xl font-bold text-white mb-1">CSV ⇆ JSON Transformer Traffic</h3>
                <p class="text-gray-400 text-sm font-medium">Monitoring visits and metrics logs for the CSV ⇆ JSON converter utility.</p>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-850/60 text-sm">
                            <tr v-for="log in recentVisits" :key="log.id" class="hover:bg-gray-800/20 transition-colors group">
                                <td class="py-4 font-mono text-xs font-bold text-indigo-400">
                                    {{ log.ip_address }}
                                </td>
                                <td class="py-4">
                                    <div v-if="log.user">
